...
public function database_view( $section = "Linux News Videos" ) {
     global $wpdb;
     $db_sites=[];
	 $cnt=0;
     $row="";
     $rqdb="SELECT * FROM `ap_section_feeds` WHERE section LIKE '".$section."';";  
     //$rqcnt="SELECT * FROM ap_section_feeds WHERE section ='Linux News Videos';";  
     
     foreach( $wpdb->get_results( $rqdb ) as $key => $row) {
		$db_sites[$cnt] = $row->site;
        $db_sites[$cnt+1] = $row->last_post_date;
		$cnt=$cnt+2;
	}
